...
public class FavoriteCommentPostEnterpriseController(
    ITokenService tokenService,
    IUserService userService,
    IPostUserService postUserService,
    ICommentPostEnterpriseService commentPostEnterpriseService,
    IFavoriteCommentPostEnterpriseService favoriteCommentPostEnterpriseService,
    ...
    public async Task<IActionResult> Exists(string commentId)
    {
        try
        {
            ...
            if (!await commentPostEnterpriseService.ExistsById(commentId))
            {
            ...
            bool favorite = await favoriteCommentPostEnterpriseService.ExistsByCommentIdAndUserId(commentId, userId);
    ...
    public async Task<IActionResult> Toggle(string commentId)
    {
        try
        {
            ...
            var favorite = await favoriteCommentPostEnterpriseService.GetByCommentIdAndUserId(commentId, userId);
            if (favorite != null)
            {
                await favoriteCommentPostEnterpriseService.Delete(favorite);
            ...
            await favoriteCommentPostEnterpriseService.Create(commentId, userId);
    ...
    [HttpGet]
    [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Page<FavoriteCommentPostEnterpriseDto>))]
    [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ResponseHttp<object>))]
    public async Task<IActionResult> GetAll([FromQuery] FavoriteCommentFilter filter )
    {
        IQueryable<FavoriteCommentEntity> query = favoriteCommentPostEnterpriseService.GetAllQuery();
        ...
        Page<FavoriteCommentPostEnterpriseDto> dtos = mapper.Map<Page<FavoriteCommentPostEnterpriseDto>>(paginatedList);
